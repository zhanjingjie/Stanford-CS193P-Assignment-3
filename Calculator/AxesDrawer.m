...
/*
 Draw the mark number, at point, and anchor position
 */
...
+ (void)drawHashMarksInRect:(CGRect)bounds originAtPoint:(CGPoint)axisOrigin scale:(CGFloat)pointsPerUnit
{
	...
	// Check the axis origin if it is meaningful, origin's x and y value should be within the bounds
	...
	// unitsPerHashmark is the distance between every two marks
	for (int offset = unitsPerHashmark; !started || stillGoing; offset += unitsPerHashmark)
	{
		...
		// This is drawing the horizontal line, the positive hashmark axis
		hashMarkPoint.x = axisOrigin.x+scaledOffset;
		hashMarkPoint.y = axisOrigin.y;
		
		// Check if the hashmark is still within the bounds (the screen)
		if (CGRectContainsPoint(bounds, hashMarkPoint)) {
			
			// Add a small line across the x axis to be the mark
			...
			// Anchor top means the number will be drew below the mark line, with number offset
	...
	// Draw all of them at the same time, cool!
...
+ (void)drawAxesInRect:(CGRect)bounds originAtPoint:(CGPoint)axisOrigin scale:(CGFloat)pointsPerUnit
{
	...
	// This can draw the horizontal and vertical axis without marks
	CGContextBeginPath(context);
